...
class BuildingCol:
    ...
    def __init__(self,nz,dz,dt,vol,lambdap,lambdaf,hmean,Ck,Cp,th_ref,vx,vy,th,Cdrag,rho,nz_u,pb,ss
        ...
        for i in range(coordination.EP_nFloor):
            ...
            # else:
            #     self.SensHt_HVAC_Floor[int(centroid_idices[i])] = sum(coordination.EP_floor_energy_lst[:i + 1]) * 2E-2
            # multiplier = 1E-2 if i < 30 else 1
            # # print(sum(coordination.EP_floor_energy_lst[:i + 1]), end=',')
            multiplier = 1 if i < 20 else 1
            self.SensHt_HVAC_Floor[int(centroid_idices[i])] = sum(coordination.EP_floor_energy_lst[:i + 1]) * multiplier
    ...
    def BuildingDrag_UTC(self,thb,qhb,tvb,FractionsGround,FractionsRoof,TWallSun,TWallShade,TGround,TRoof):
        ...
        for i in range(0, self.nz_u + 1):
            ...
            # Term in energy equation [K s^-1]
            # print(self.SensHt_HVAC)
            # print(self.SensHt_HVAC_Floor)
            if 'vector' in coordination.value:
                ...
                self.srex_th_h[i] += (thb_Roofs_Imp / self.nd) * (self.ss[i] * self.lambdap / self.vol[i] / self.dz) \
                                 ...
                                 self.HVAC_atm_frac*(1-self.HVAC_street_frac)*\
                                     (self.SensHt_HVAC /(self.rho[i]*self.Cp)/self.dz)*self.ss[i]*self.lambdap/(1 - self.lambdap)
