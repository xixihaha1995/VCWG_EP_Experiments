...
import numpy
def ini_all(_bld_type,_experiments_theme,_idfFileName,_epwFileName,_start_time,
            _TopForcingFileName,_VCWGParamFileName,_sensor_height_meter):
    global project_path, save_path_clean, sensor_heights,ep_trivial_path, data_saving_path, bld_type,\
    ...
    ep_indoorTemp_C, ep_sensWaste_w_m2_per_footprint_area, \
        ...
        footprint_area_m2, vcwg_hConv_w_m2_per_K, EP_nFloor, EP_floor_energy_lst, EP_wall_temperatures_K_dict, \
...
def BEMCalc_Element(BEM, it, simTime, VerticalProfUrban, Geometry_m, MeteoData,
    ...
    if not os.path.exists(data_saving_path):
        ...
        with open(data_saving_path, 'a') as f1:
            ...
            header_str = 'cur_datetime,canTemp,sensWaste,'
    ...
    with open(data_saving_path, 'a') as f1:
        fmt1 = "%s," * 1 % (cur_datetime) + \
               "%.3f," * 2 % (vcwg_canTemp_K, BEM_Building.sensWaste,) + \
